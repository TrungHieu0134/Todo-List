...
def signup():
    if request.method == "POST":
        ...
        if user:
            ...
            flash("Địa chỉ email không hợp lệ.", category="error")
        elif len(password) != 8:
...
def profile():
    if request.method == "POST":
        ...
        # Xử lý đổi mật khẩu
            ...
            # Ngăn chặn user OAuth đổi mật khẩu
            if current_user.provider != 'local':
                flash(f"Bạn không thể thay đổi mật khẩu vì đã đăng nhập bằng {current_user.provider.title()}.", category="error")
            else:
